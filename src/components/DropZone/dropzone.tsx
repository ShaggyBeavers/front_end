...
const FileUpload: FC<FileUploadProps> = ({ onFilesChange, initialFiles }) => {
    ...
    return (
        <div className="report_input" style={{ width: '32%' }}>
            ...
            >
                <p>Завантажте до 3х фото</p>
            </label>
            <div
                {...getRootProps()}
                className={`dropzone ${isUploadDisabled ? 'disabled' : ''}`}
            >
                <input {...getInputProps()} disabled={isUploadDisabled} />
                <p
                    className={`upload-input ${isUploadDisabled ? 'disabled-upload' : ''}`}
                >
                    Завантажити фото
                </p>
            </div>
                ...
                {initialFiles.map((file, index) => (
                    <div key={index} className="uploaded-file">
                        ...
                        <button
                            ...
                            onClick={() =>
                                onFilesChange(
                                    initialFiles.filter((_, i) => i !== index)
                                )
                            }
                        >
